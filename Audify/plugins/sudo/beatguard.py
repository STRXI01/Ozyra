...
kick_words = ["kick", "out", "fuckoff", "nikal"]
promote_words = ["promote", "adminship"]
fullpromote_words = ["fullpromote", "fulladmin"]
demote_words = ["demote", "limit"]

# ❖ Usage Help Text
usage_text = """
<b>🔧 Delta's Special Admin Command Usage:</b>

Reply to a user's message and use any of the following:
• <code>/ban</code> – Ban the user
• <code>/unban</code> – Unban the user
• <code>/Delta mute</code> – Mute the user
• <code>/Deltaunmute</code> – Unmute the user
• <code>/Delta kick</code> – Kick the user
• <code>/Delta promote</code> – Promote with limited rights
• <code>/Delta fullpromote</code> – Full admin rights
• <code>/Delta demote</code> – Remove admin rights

<i>⚠️ You must reply to a user's message.</i>
"""

# ❖ Audify Main Handler
@app.on_message(filters.command(["Delta"], prefixes=["/"]) & admin_filter)
