...
def get_unique_random(pool, source):
    if not pool:
        pool.extend(source)
        random.shuffle(pool)
    return pool.pop()
...
def get_random_night_message(user):
    ...
    msg = get_unique_random(night_pool, messages)
    return msg.replace("😴", get_random_night_emoji())
...
def get_random_morning_message(user):
    ...
    msg = get_unique_random(morning_pool, messages)
    return msg.replace("🌞", get_random_morning_emoji())
...
async def goodnight_command_handler(_, message):
    ...
    text = get_random_night_message(user)
...
async def goodmorning_command_handler(_, message):
    user = safe_get_user_mention(message)
    text = get_random_morning_message(user)
    await message.reply_text(text)
