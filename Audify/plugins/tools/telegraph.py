...
@app.on_message(filters.command(["tgm", "tg", "telegraph", "cb"]))
async def telegraph_upload_handler(client, message):
    if not message.reply_to_message:
        return await message.reply_text(
            "📎 <b>Please reply to a media file to upload it on Envs.sh.</b>"
        )
    ...
    if file_size > 256 * 1024 * 1024:
        return await message.reply_text("⚠️ File size exceeds the 256 MB limit of Envs.sh.")
    ...
    try:
        ...
        await status.edit_text("📤 <b>Uploading to Envs.sh...</b>")
        ...
        if success:
            await status.edit_text(
                ...
                reply_markup=InlineKeyboardMarkup(
                    [
                        [InlineKeyboardButton("🔗 Open Envs.sh", url=result)],
                        [InlineKeyboardButton("❌ Close", callback_data="close_upload")]
                    ]
                ),
            )
        else:
